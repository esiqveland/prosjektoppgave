...
As we can see from Table \ref{tab:cluster_reqwattnode} we have a steady increase in requests we can service per watt every time we add a node to the cluster until we reach five workers. At this point the load distributor is at max capacity and we are unable to utilize the workers fully.
...
\centering
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Plot showing the number of queries we can service per Watt with increasing number of workers.}
\end{figure}
